...
func powershell(t *testing.T) string {
	...
	// Skipping locally is a convenience. Skipping in CI would be a silent hole:
	// install.ps1 is the one installer no other job can execute, so a skip there
	// leaves its guard permanently unverified while the suite still reports
	// green — which is how this flaw survived in the first place. Runners all
	// ship pwsh, so its absence means the environment changed and we should hear
	// about it, not sail past.
	if os.Getenv("CI") != "" {
		t.Fatalf("pwsh not found in CI: install.ps1 would go untested (%v)", err)
	}
